        ...
        with open(ARCHIVO_CSV, newline="", encoding="utf-8-sig") as archivo:
            ...
            for numero_fila, fila in enumerate(lector, start=2):
                ...
                if not all(fila.get(campo, "").strip() for campo in campos_requeridos):
                    print(
                        f"[AVISO] Fila {numero_fila} incompleta, se omite: {dict(fila)}"
                    )
                    continue
                ...
                try:
                    ...
                    print(
                        f"[ERROR] Fila {numero_fila}: poblacion/superficie deben ser "
...
def pedir_texto(mensaje):
    while True:
        entrada = input(mensaje).strip()
        if entrada:
            if not entrada.isalpha() or " " in entrada:
                print("  [!] El campo no puede contener números o símbolos.")
                continue
            return entrada
        print("  [!] El campo no puede estar vacío.")
...
def agregar_pais(paises):
    ...
    superficie = pedir_entero(
        "  Superficie en km² (número entero positivo): ", minimo=1
    )
...
def actualizar_pais(paises):
    ...
    entrada_pob = input(
        f"  Nueva población [{pais['poblacion']:,}]: ".replace(",", ".")
    ).strip()
    if entrada_pob:
        try:
            ...
            if nueva_pob < 1:
                print(
                    "error! La población debe ser un entero positivo. No se actualizó."
                )
            else:
    ...
    entrada_sup = input(
        f"  Nueva superficie [{pais['superficie']:,} km²]: ".replace(",", ".")
    ).strip()
    if entrada_sup:
        try:
            ...
            if nueva_sup < 1:
                print(
                    " error! La superficie debe ser un entero positivo. No se actualizó."
                )
            else:
...
def mostrar_estadisticas(paises):
    ...
    print("  POBLACIÓN")
    print(
        f"    Mayor : {pais_mayor_pob['nombre']} "
        f"({pais_mayor_pob['poblacion']:,})".replace(",", ".")
    )
    print(
        f"    Menor : {pais_menor_pob['nombre']} "
    ...
    print("  SUPERFICIE")
    print(
        f"    Mayor : {pais_mayor_sup['nombre']} "
        f"({pais_mayor_sup['superficie']:,} km²)".replace(",", ".")
    )
    print(
        f"    Menor : {pais_menor_sup['nombre']} "
    ...
    for continente, cantidad in sorted(
        ...
        print(f"    {continente:<15} : {cantidad:>3}")
...
while True:
    # Menú principal directamente en el bucle
    imprimir_separador("═")
